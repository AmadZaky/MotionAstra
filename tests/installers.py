...
class InstallerTests(unittest.TestCase):
 ...
 def run_install(self,answer='yes\n',preamble=''):
  ...
  return subprocess.run(['bash','-c','source "$1"; '+(preamble+'; ' if preamble else '')+'if ma_install "$2" "$3" "$4" "$5"; then exit 0; else exit $?; fi','test',str(ROOT/'install-macos.command'),str(self.payload),str(self.user),str(self.backups),str(self.system)],input=answer,text=True,capture_output=True)
 def test_clean(self):
 ...
 def test_decline_keeps_old(self):
  old=self.old();r=self.run_install('no\n');self.assertEqual(r.returncode,2,r.stdout+r.stderr);self.assertTrue((old/'sentinel').exists());self.assertFalse((self.user/'MotionAstra-FX').exists())
 ...
 def test_failed_activation_rolls_back(self):
  old=self.old();r=self.run_install(preamble='mv(){ if [[ "$1" == */new ]]; then return 1; fi; command mv "$@"; }');self.assertNotEqual(r.returncode,0);self.assertTrue((old/'sentinel').exists());self.assertFalse((self.user/'MotionAstra-FX').exists())
