...
for(const id of ['counter','stretch','neongrid']){
 ...
 const l=e.comp.selectedLayers[0];l.fx.property('MA2 loopMode').remove();const old=l.fx.addProperty('ADBE Checkbox Control');old.name='MA2 loop';old.property(1).value=0;l.comment=l.comment.replace(encodeURIComponent('2.8.2'),'old-build');
