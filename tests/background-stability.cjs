...
{const e=create();e.rpc({action:'generateBackground',id:'neongrid',params:{}});const l=e.comp.selectedLayers[0];l.comment=l.comment.replace(encodeURIComponent('2.8.2'),'old-build');const first=l.masks.property(1);assert.equal(e.rpc({action:'update',id:'neongrid',params:{}}).changed,1);assert.notEqual(l.masks.property(1),first);const migrated=l.masks.property(1);e.rpc({action:'update',id:'neongrid',params:{}});assert.equal(l.masks.property(1),migrated);}
