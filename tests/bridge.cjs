...
(async () => {
  for (const drop of [undefined, '', 'undefined', 'null', 'EvalScript error.']) {
    ...
    assert.equal(status.hostVersion, '2.8.4');
  ...
  const oldBuild=environment();oldBuild.host.MotionAstra={version:'2.8.4',build:'previous',dispatch(){throw Error('Stale host reused');}};
