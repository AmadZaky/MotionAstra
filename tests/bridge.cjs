...
(async () => {
  for (const drop of [undefined, '', 'undefined', 'null', 'EvalScript error.']) {
    ...
    assert.equal(status.hostVersion, '2.8.3');
  ...
  const oldBuild=environment();oldBuild.host.MotionAstra={version:'2.8.3',build:'previous',dispatch(){throw Error('Stale host reused');}};
